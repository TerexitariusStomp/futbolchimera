...
export default function App() {
  ...
  const handleWebViewMessage = async (event) => {
    try {
      ...
      if (msg.type === 'wallet-connected') {
        ...
        try {
          Linking.openURL('io.localchimera.mobile://wallet').catch(() => { });
        } catch (e) { }
        return;
      }
  ...
  const injectedBridge = `
    (function() {
      ...
      window.__apiOrigin = ${JSON.stringify(machineId ? 'https://localchimera.local/' + machineId : 'https://localchimera.local')};
